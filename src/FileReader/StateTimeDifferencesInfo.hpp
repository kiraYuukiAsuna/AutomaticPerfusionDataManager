...
struct StateTimeDifferenceInfo {
    ...
    friend void from_json(const nlohmann::json&jsonObject, StateTimeDifferenceInfo&dataObject) {
        ...
        dataObject.MOVE_TO_CELL = jsonObject.value("MOVE_TO_CELL", defaultDataObject.MOVE_TO_CELL);
        dataObject.AUTO_CALIBRATE = jsonObject.value("AUTO_CALIBRATE", defaultDataObject.AUTO_CALIBRATE);
        dataObject.RETREAT_NEEDLE = jsonObject.value("RETREAT_NEEDLE", defaultDataObject.RETREAT_NEEDLE);
        dataObject.FIND_CELL = jsonObject.value("FIND_CELL", defaultDataObject.FIND_CELL);
        dataObject.LOWER_NEEDLE = jsonObject.value("LOWER_NEEDLE", defaultDataObject.LOWER_NEEDLE);
        dataObject.AXIAL_INJECTCELL = jsonObject.value("AXIAL_INJECTCELL", defaultDataObject.AXIAL_INJECTCELL);
        dataObject.START_PERFUSE = jsonObject.value("START_PERFUSE", defaultDataObject.START_PERFUSE);
        dataObject.WAIT = jsonObject.value("WAIT", defaultDataObject.WAIT);
        dataObject.WITHDRAW_NEEDLE = jsonObject.value("WITHDRAW_NEEDLE", defaultDataObject.WITHDRAW_NEEDLE);
        dataObject.LIFT_NEEDLE = jsonObject.value("LIFT_NEEDLE", defaultDataObject.LIFT_NEEDLE);
...
inline StateTimeDifferenceInfoList ReadStateTimeDifferenceInfoListFromFile(const std::filesystem::path&filePath) {
