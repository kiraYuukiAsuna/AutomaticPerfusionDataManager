...
void OverviewPage::getStatus() {
	...
	for (const auto &row : results) {
		series->append(QString::fromStdString(std::get<0>(row)),
					   std::get<1>(row));
...
void OverviewPage::getFluorescenceResult() {
    ...
    m_FluorescenceResultChartView->chart()->setAnimationOptions(QChart::AllAnimations);
...
void OverviewPage::calculateSuccessRate() {
    ...
    auto fluorescenceResults = db.select(
        columns(&CellTissueInfo::FluorescenceResult, count(&CellTissueInfo::FluorescenceResult)),
        group_by(&CellTissueInfo::FluorescenceResult));
    ...
    double successRate = validTotalCount > 0 ? static_cast<double>(successCount) / validTotalCount : 0.0;

    std::string successRateText = "有效细胞数量：" + std::to_string(validTotalCount) + "，成功灌注细胞数量：" + std::to_string(successCount) + " | 成功率：" + std::to_string(successRate * 100) + "%";

    m_SuccessRateText = new ElaText(QString::fromStdString(successRateText), this);
}
...
void OverviewPage::calculateTimeRange() {
    m_TimeRange = new ElaText("已统计数据时间范围：xxxx.xx.xx - xxxx.xx.xx",this);

    using namespace sqlite_orm;
    auto db = SqliteSchema::getDBStorage();

    // Retrieve all PerfusionDate and PerfusionTime from the database
    auto results = db.select(columns(&CellTissueInfo::PerfusionDate, &CellTissueInfo::PerfusionTime));
    ...
    timeStream << "从 " << std::put_time(&minTimePoint, "%Y-%m-%d %H:%M:%S") << " 至 " << std::put_time(&maxTimePoint, "%Y-%m-%d %H:%M:%S");
    std::string perfusionTime = timeStream.str();

    m_TimeRange->setText(QString::fromStdString("（测试数据）已统计数据时间范围：" + perfusionTime));
}
...
void OverviewPage::plotPerfusionResults(){
    ...
    auto results = db.select(columns(&CellTissueInfo::PerfusionDate, &CellTissueInfo::FluorescenceResult));
    ...
    chart->addSeries(successSeries);
    chart->addSeries(failureSeries);
    chart->setTitle("Daily Perfusion Success and Failure Counts");
    ...
    m_PerfusionChartView = new QChartView(chart, this);
